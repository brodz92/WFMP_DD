...
public class ShiftTemplateImportService
{
    // TODO: confirm endpoint
    private const string Endpoint = "/wfd/replaceme/shift-templates";
        ...
        IEnumerable<CsvRecord> rows, WfdApiClient api, string tenantId, int concurrency = 6, CancellationToken ct = default)
    {
        ...
        {
            var row = raw is ShiftTemplateRecord pr ? pr : new ShiftTemplateRecord { };
            ...
            tasks.Add(Task.Run(async () => {
                try
                {
                    var payload = ShiftTemplateMapper.ToPayload(row, tenantId);
